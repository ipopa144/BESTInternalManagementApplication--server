...
public class KMeans {
    ...
    private static Map<CentroidDTO, List<Record>> finalClusters;
     ...
     */
    public static Map<CentroidDTO, List<Record>> fit(List<Record> records, Distance distance, int maxIterations) {

        applyPreconditions(records, k, distance, maxIterations);

        List<CentroidDTO> centroids = randomCentroids(records, k);
        Map<CentroidDTO, List<Record>> clusters = new HashMap<>();
        ...
        for (int i = 0; i < maxIterations; i++) {
            ...
            for (Record record : records) {
                CentroidDTO centroid = nearestCentroid(record, centroids, distance);
                assignToCluster(clusters, record, centroid);
            }

            // if the assignment does not change, then the algorithm terminates
            boolean shouldTerminate = isLastIteration || clusters.equals(finalClusters);
     ...
     */
    private static List<CentroidDTO> relocateCentroids(Map<CentroidDTO, List<Record>> clusters) {
     ...
     */
    private static CentroidDTO average(CentroidDTO centroid, List<Record> records) {
        ...
        return new CentroidDTO(average, null, null);
    }
     ...
     */
    private static void assignToCluster(Map<CentroidDTO, List<Record>> clusters, Record record, CentroidDTO centroid) {
     ...
     */
    private static CentroidDTO nearestCentroid(Record record, List<CentroidDTO> centroids, Distance distance) {
        double minimumDistance = Double.MAX_VALUE;
        CentroidDTO nearest = null;

        for (CentroidDTO centroid : centroids) {
     ...
     */
    private static List<CentroidDTO> randomCentroids(List<Record> records, int k) {
        List<CentroidDTO> centroids = new ArrayList<>();
        ...
        for (int i = 0; i < k; i++) {
            ...
            centroids.add(new CentroidDTO(coordinates, null, null));
        }
    ...
    /**
     * Computes the Mean Max Distance value for each cluster and sets it accordingly
     *
     * @param distance The distance which is used to compute the mean max distance
     */
    public static void computeMeanMaxDistance(Distance distance) {

        AtomicReference<Double> sum = new AtomicReference<>((double) 0);
        finalClusters.forEach((centroid, records) -> {
            double maxDistance = Double.MIN_VALUE;

            for(int i = 0; i < records.size() - 1; i++)
                for(int j = i + 1; j < records.size(); j++) {
                    double distanceValue = distance.calculate(records.get(i).getFeatures(), records.get(j).getFeatures());
            ...
            sum.updateAndGet(v -> (double) (v + finalMaxDistance));
        });
